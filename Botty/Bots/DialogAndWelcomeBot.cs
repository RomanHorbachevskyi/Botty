            ...
            foreach (var member in membersAdded)
            {
                ...
                if (member.Id != turnContext.Activity.Recipient.Id)
                {
                    ...
                    var response = ((Activity)turnContext.Activity).CreateReply(
                        "Hello, we are happy that you are using our service!" +
                        "\nTo start conversation type anything, please.");
                    //response.Attachments = new List<Attachment>() {welcomeCard};

                    await turnContext.SendActivityAsync(response, cancellationToken);
                    //await turnContext.SendActivityAsync(response, cancellationToken);

                    //return stepContext.PromptAsync(nameof(TextPrompt), new PromptOptions { Prompt = MessageFactory.Text("Hello!\nWhat do you want to book,\ntour or hotel?") }, cancellationToken);
                }
            }
        }
